...
settle together and are not independent evidence.
  ...
  MARGINAL DEPTH  pnl per share by depth-in-ticks above the touch, with
                  decision-ordered time halves. A depth that loses in both
                  halves is a pure donation.
  CAP REPLAY      what each cap (touch + X ticks) would have done on the
                  same decisions: shares kept, $ deployed, EV c/share and
                  $/day. Capping wins in ABSOLUTE terms only where the cut
                  tail was negative -- a positive tail still adds dollars
                  even at worse per-share EV, and $/day is the objective.
...
def unit(path):
    ...
    for cap in CAPS + (999,):
        kept = [x for _, r in decs for x in r if x[0] <= cap]
        sz = sum(x[2] for x in kept)
        pnl = sum(x[5] for x in kept)
        cost = sum(x[1] * x[2] for x in kept)
        lab = "uncap" if cap == 999 else f"+{cap}"
        delta = "" if cap == 999 else f" {(pnl - unc) / days:>+9.2f}"
        print(f"{lab:>7} {sz/len(decs):>7.1f} {cost/len(decs):>7.2f} "
              f"{100*pnl/sz:>+8.2f} {pnl/days:>+8.2f}{delta}")
    ...
    for d in (1, 2, 3, 6):
        ...
        if min(n1, n2) < MIN_DEC:
            print(f"  verdict: tail +{d}+ has only {a['n']} decisions -- "
        ...
        if c1 < 0 and c2 < 0:
            print(f"  verdict: every share {d}+ ticks above the touch LOSES "
                  ...
                  f"decisions) -- capping at touch+{d-1} is actionable as a "
